<script>
    ...
    async function resolveIcon(url) {
        ...
        if (iconCache[url]) {
            resolvedIcon = iconCache[url];
            return;
        }
        try {
            const dataUrl = await invoke("fetch_icon", { url });
            if (dataUrl) iconCache[url] = dataUrl;
            if (currentGame?.icon === url) resolvedIcon = dataUrl || null;
        } catch (_) {
            if (currentGame?.icon === url) resolvedIcon = null;
        }
    }

    $effect(() => {
        const url = currentGame?.icon ?? null;
        if (!url) {
            resolvedIcon = null;
            return;
        }
    ...
    onMount(async () => {
        ...
        unlisten.push(
            await listen("log", ({ payload }) => {
                addLog(payload.level, payload.message);
            }),
        );

        unlisten.push(
            await listen("ws_status", ({ payload }) => {
                wsStatus = payload;
        ...
        unlisten.push(
            await listen("rpc_status", ({ payload }) => {
                rpcStatus = payload;
                addLog(
                    payload.connected ? "success" : "warn",
                    `Discord: ${payload.message}`,
                );
            }),
        );

        unlisten.push(
            await listen("presence_event", ({ payload }) => {
                if (payload.type === "presence") {
                    ...
                    addLog("info", `▶ ${payload.name} (${titleId})`);
        ...
        unlisten.push(
            await listen("install_complete", ({ payload }) => {
        ...
        unlisten.push(
            await listen("uninstall_complete", () => {
                status = {
                    ...status,
                    installed: false,
                    token: null,
        ...
        async function pollLiveStatus() {
            try {
                ...
                wsStatus = {
                    connected: live.ws_connected,
                    message: live.ws_message,
                };
                rpcStatus = {
                    connected: live.rpc_connected,
                    message: live.rpc_message,
                };
                if (live.current_game) {
                    currentGame = {
                        name: live.current_game.name,
                        icon: live.current_game.icon,
                        titleId: live.current_game.title_id,
                    };
    ...
    function addLog(level, message) {
        ...
        logs = [{ id: ++logId, ts, level, message }, ...logs].slice(
            0,
            MAX_LOGS,
        );
    }
    ...
    async function toggleAutostart() {
        try {
            ...
            addLog("error", `Autostart toggle failed: ${e}`);
        }
    }
...
<main>
    <header>
        ...
        <div class="badges">
            <div
                class="status-pill"
                class:on={wsStatus.connected}
                title={wsStatus.message}
            >
                <span class="pip"></span><span>Server</span>
            </div>
            <div
                class="status-pill"
                class:on={rpcStatus.connected}
                title={rpcStatus.message}
            >
                <svg
                    class="discord-icon"
                    viewBox="0 0 24 24"
    ...
    <nav>
        ...
        >
            <svg viewBox="0 0 20 20" fill="currentColor"
                ><path
                    fill-rule="evenodd"
                    d="M3 5a1 1 0 0 1 1-1h12a1 1 0 1 1 0 2H4a1 1 0 0 1-1-1zm0 5a1 1 0 0 1 1-1h12a1 1 0 1 1 0 2H4a1 1 0 0 1-1-1zm1 4a1 1 0 1 0 0 2h6a1 1 0 1 0 0-2H4z"
                    clip-rule="evenodd"
                /></svg
            >
            Logs {#if logs.length > 0}<span class="log-count"
                    >{logs.length}</span
                >{/if}
        </button>
        <button
        ...
        <section class="tab">
            ...
            {:else}
                <div class="idle-card">
                    <div class="idle-icon">
                        <svg viewBox="0 0 48 48" fill="none"
                            ><circle
                                ...
                                opacity="0.3"
                            /><path
                                d="M16 24h4m8 0h4M24 16v4m0 8v4"
                                stroke="#7289da"
                                stroke-width="2"
                    ...
                    <div class="idle-sub">
                        Launch a game on your 3DS with the plugin enabled
                    </div>
                </div>
            {/if}
            ...
            <div class="status-grid">
                ...
                <div class="stat-card" class:stat-ok={rpcStatus.connected}>
                    ...
                    <div class="stat-value">
                        {rpcStatus.connected ? "Connected" : "Not open"}
                    </div>
                </div>
            </div>
                ...
                {:else}
                    ...
                    >
                        <svg viewBox="0 0 20 20" fill="currentColor"
                            ><path
                                fill-rule="evenodd"
                                d="M9 2a1 1 0 0 0-.894.553L7.382 4H4a1 1 0 0 0 0 2v10a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V6a1 1 0 1 0 0-2h-3.382l-.724-1.447A1 1 0 0 0 11 2H9zM7 8a1 1 0 0 1 2 0v6a1 1 0 1 1-2 0V8zm4 0a1 1 0 0 1 2 0v6a1 1 0 1 1-2 0V8z"
                                clip-rule="evenodd"
                            /></svg
                        >
                        Uninstall
    ...
    {#if activeTab === "settings"}
        <section class="tab settings-tab">
            <div class="card">
                <div class="card-header"><h3>Startup</h3></div>
                <div class="autostart-row">
                    <div class="autostart-label">
                        <span class="label-text">Launch at login</span>
                        <span class="autostart-hint"
                            >Start in tray automatically when you log in</span
                        >
                    </div>
                    <button
                        class="toggle"
            ...
            <div class="card">
                ...
                <p class="settings-hint">
                    Server settings are managed by your token provisioning.
                </p>
            </div>
            <div class="card">
...
<style>
    ...
    .btn.primary {
        background: linear-gradient(135deg, #7289da, #5b6fc7);
        color: #fff;
        box-shadow: 0 2px 12px #7289da33;
    }
    .btn.primary:hover:not(:disabled) {
        transform: translateY(-1px);
        box-shadow: 0 4px 20px #7289da44;
    }
    .btn.secondary {
        background: #1a1d26;
        color: #7289da;
        border: 1px solid #7289da44;
    }
    ...
    .log-line {
        ...
        gap: 10px;
        padding: 4px 16px;
        border-bottom: 1px solid #0d0f1488;
    ...
    .log-line.info .log-level-dot {
        background: #4a4f6a;
    }
    .log-line.success .log-level-dot {
        background: #3ba55d;
    }
    .log-line.warn .log-level-dot {
        background: #faa61a;
    }
    .log-line.error .log-level-dot {
        background: #f04747;
    }
    .log-msg {
        color: #888;
